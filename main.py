...
def start(stdscr) -> None:
    ...
    while True:
        ...
        win.addstr(int(rows/2)-8, int(cols/2)-12, "[ 1 ] Hiragana "+u"\u2192"+" Romaji", curses.A_REVERSE if train_method == 0 else curses.A_NORMAL)
        win.addstr(int(rows/2)-6, int(cols/2)-12, "[ 2 ] Katakana "+u"\u2192"+" Romaji", curses.A_REVERSE if train_method == 1 else curses.A_NORMAL)
        win.addstr(int(rows/2)-4, int(cols/2)-12, "[ 3 ] Romaji "+u"\u2192"+" Hiragana", curses.A_REVERSE if train_method == 2 else curses.A_NORMAL)
        win.addstr(int(rows/2)-2, int(cols/2)-12, "[ 4 ] Romaji "+u"\u2192"+" Katakana", curses.A_REVERSE if train_method == 3 else curses.A_NORMAL)
    ...
    for iterations in range(training_length):
        win.nodelay(True)
        draw_standard_screen(win, rows, cols)
        kana = training_list.pop()
        win.addstr(int(rows/2), int(cols/2), kana, curses.A_NORMAL)
        win.nodelay(False)
        key = win.getch()
        if key == ord("q"):
            try:
